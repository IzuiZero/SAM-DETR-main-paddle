...
def main(args):
    ...
    optimizer = paddle.optimizer.AdamW(param_dicts, lr=args.lr, weight_decay=args.weight_decay)
    lr_scheduler = paddle.optimizer.lr.StepDecay(optimizer, step_size=args.lr_drop, gamma=0.1)
    ...
    if args.frozen_weights is not None:
        ...
        model_without_ddp.detr.set_state_dict(checkpoint['model'])
    ...
    if args.eval:
        ...
        if args.output_dir:
            utils.save_on_master(coco_evaluator.coco_eval["bbox"].eval, output_dir / "eval.pdparams")
        return
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir:
            ...
            if (epoch + 1) % args.save_every_epoch == 0:
                checkpoint_paths.append(output_dir / f'checkpoint{epoch:04}.pdparams')
        ...
        if (epoch + 1) % args.eval_every_epoch == 0:
            ...
            if coco_evaluator is not None:
                ...
                if "bbox" in coco_evaluator.coco_eval:
                    filenames = ['latest.pdparams']
                    filenames.append(f'{epoch:04}.pdparams')
                    for name in filenames:
                        paddle.save(coco_evaluator.coco_eval["bbox"].eval, output_dir / "eval" / name)
