...
def crop(image, target, region):
    ...
    if "boxes" in target or "masks" in target:
        ...
        if "boxes" in target:
            ...
            keep = paddle.any(target['masks'].flatten(1), axis=1)
...
def hflip(image, target):
    ...
    if "masks" in target:
        target['masks'] = paddle.flip(target['masks'], axis=[-1])
...
def resize(image, target, size, max_size=None):
    ...
    rescaled_image = image.resize(size, Image.BILINEAR)
...
def pad(image, target, padding):
    ...
    padded_image = paddle.nn.functional.pad(image, [0, 0, padding[0], padding[1]])
    if target is None:
